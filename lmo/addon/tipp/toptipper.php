...
$output_gotoLMO.=$text['tipp'][52];
$template->setVariable("LetzteAuswertung", $output_letzteauswertung);
$template->setVariable("gotoTipp", $output_gotoTippspiel);
$template->setVariable("gotoLMO", $output_gotoLMO);

if ($multi_cfgarray['all']!=1){
  $template->setVariable("URLTipp", URL_TO_LMO.'/lmo.php?action=tipp&amp;&todo=wert&amp;file='.$multi_cfgarray['file']);
  $template->setVariable("URLLMO", URL_TO_LMO.'/lmo.php?todo=&file='.$multi_cfgarray['file']);
} else {
  $template->setVariable("URLTipp", URL_TO_LMO.'/lmo.php?action=tipp');
  $template->setVariable("URLLMO", URL_TO_LMO.'/lmo.php');
}
//$template->parse();
$template->show();
?>
